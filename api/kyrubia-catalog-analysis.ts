  ...
  try {
    response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: analysisSystemInstruction({ uid: '', name: 'Usuário do Kyrub' }, 'Análise de catálogo') }] },
          contents,
...
const callGeminiWithFallback = async (
  ...
  try {
    return {
      payload: await callGemini(apiKey, preferredModel, contents, controller),
    ...
    return {
      payload: await callGemini(apiKey, fallbackModel, contents, controller),
      model: fallbackModel,
      fallbackUsed: true,
    };
  }
