...
const modalSource = readFileSync('src/components/store/UnifiedProductModal.tsx', 'utf8');
const editorSource = readFileSync('src/components/store/ProductInventoryCompositionEditor.tsx', 'utf8');
const purchaseSource = readFileSync('src/components/store/ProductPurchaseList.tsx', 'utf8');
const inventorySource = readFileSync('src/utils/productInventory.ts', 'utf8');
const publicProductsSource = readFileSync('src/utils/publicProducts.ts', 'utf8');
const reconciliationSource = readFileSync('server/inventory/productStockReconciliationService.ts', 'utf8');
const transformationSource = readFileSync('server/inventory/inventoryTransformationExecutionService.ts', 'utf8');
const transformationClientSource = readFileSync('src/actions/inventoryTransformationActionService.ts', 'utf8');
const actionEventsSource = readFileSync('src/ai/actionEvents.ts', 'utf8');
const inventoryBridgeSource = readFileSync('src/components/KyrubAiInventoryActionBridge.tsx', 'utf8');
...
const catalog: InventoryCatalogItem[] = [
  { id: 'flour', name: 'Farinha', unit: 'g', currentQuantity: 5000, minimumQuantity: 2000, purchaseCost: 0.01, supplier: 'Fornecedor A', updatedAt: '' },
  { id: 'cheese', name: 'Queijo', unit: 'g', currentQuantity: 900, minimumQuantity: 1200, purchaseCost: 0.04, supplier: '', updatedAt: '' },
];
...
describe('product inventory and composition', () => {
  test('parses a private reusable catalog with supported base units', () => {
    ...
    assert.deepEqual(parseInventoryCatalog([...catalog, { id: 'invalid/id', name: 'Inválido', unit: 'kg' }]), catalog);
  });

  test('calculates sellable stock from recipe consumption and yield', () => {
    const composition = parseProductComposition({ kind: 'recipe', yieldQuantity: 10, lines: [
  ...
  test('uses the same composition model for retail bundles', () => {
    const composition = parseProductComposition({ kind: 'bundle', yieldQuantity: 1, lines: [
  ...
  test('inventory stays owner-only instead of entering public product payloads', () => {
    assert.equal(getProductInventoryDocumentPath('owner-a'), 'users/owner-a/private_store/inventory');
    assert.match(inventorySource, /private_store\/inventory/);
  ...
  test('authoritative inventory adjustments reconcile sellable stock without publishing recipes', () => {
    ...
    assert.match(actionExecuteSource, /executeAuthorizedKyrubInventoryAdjustment[\s\S]*reconcileDerivedProductStockForTenant/);
  });

  test('universal transformation contract supports intermediates, byproducts and bounded audited losses', () => {
    const proposal = normalizeKyrubInventoryTransformationProposal({
      id: 'batch-beef-001', type: 'transform_inventory',
      ...
      losses: [{ name: 'Gordura e limpeza', quantity: 300, unit: 'g', reason: 'descarte' }],
    ...
    assert.equal(normalizeKyrubInventoryTransformationProposal({
      id: 'impossible-loss', type: 'transform_inventory',
      inputs: [{ name: 'Carne bovina', quantity: 5, unit: 'kg' }],
      outputs: [{ name: 'Hambúrguer', quantity: 1, unit: 'un', kind: 'intermediate' }],
      losses: [{ name: 'Perda', quantity: 6, unit: 'kg' }],
      source: { kind: 'processing' }, requiresConfirmation: true,
    }), null);
  });
  ...
  test('natural language creates a deterministic transformation without inferring missing values', () => {
    ...
    assert.ok(proposal);
    assert.deepEqual(proposal.inputs[0], { name: 'Carne bovina', quantity: 5, unit: 'kg' });
    assert.deepEqual(proposal.outputs[0], { name: 'Hambúrguer 100g', quantity: 50, unit: 'un', kind: 'intermediate' });
    assert.equal(proposal.losses[0]?.quantity, 300);
    assert.equal(
      buildKyrubInventoryTransformationProposal('Transforme carne em hambúrguer.', 'conversation-ambiguous'),
      null
    );
  });
  ...
  test('structured transformation syntax supports reusable byproducts', () => {
    ...
    assert.ok(proposal);
    assert.equal(proposal.outputs.find(item => item.kind === 'byproduct')?.name, 'Aparas aproveitáveis');
  });
  ...
  test('transformations share the existing action runtime and reconcile sellable stock', () => {
    ...
    assert.match(actionExecuteSource, /reconcileDerivedProductStockForTenant\(result\.entityId\)/);
  ...
  test('Kyrubia routes transformation before simpler inventory movements', () => {
    const transformationIndex = consultantRouteSource.indexOf('const transformation = inventoryTransformationResponse');
    const movementIndex = consultantRouteSource.indexOf('const movement = inventoryMovementResponse');
  ...
  test('browser validates, reviews and confirms transformations through the safe executor', () => {
    ...
    assert.doesNotMatch(transformationClientSource, /firebase\/firestore|setDoc\(|updateDoc\(|runTransaction\(/);
  });
});
