...
test('proposal is ready only when normal and conditional required attributes are satisfied', async () => {
  const source = await readFile(servicePath, 'utf8');
...
test('authenticated Mercado Livre POST helper sends bearer JSON requests', async () => {
...
test('conditional validation route remains owner authenticated and separate from publication', async () => {
