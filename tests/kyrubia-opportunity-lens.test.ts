...
test('Kyrubia is the named AI persona and uses a restrained opportunity lens', () => {
  ...
  assert.match(sharedSource, /transport=kyrubia-user-ai-chat/);
  assert.match(sharedSource, /KYRUB_AI_PLATFORM_CONSULTANT_ENDPOINT = '\/api\/consultor-kyrub'/);
  assert.match(sharedSource, /KYRUB_AI_CONSULTANT_COMPAT_ENDPOINT = KYRUB_AI_PLATFORM_CONSULTANT_ENDPOINT/);
