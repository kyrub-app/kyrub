...
import { verifyMercadoPagoWebhookSignatureWithSecret } from '../server/payments/mercadoPagoPixProvider';
...
test('signature verifier accepts the documented HMAC manifest without credential IO', () => {
  ...
  assert.doesNotThrow(() => verifyMercadoPagoWebhookSignatureWithSecret({
    dataId,
    headers: {
