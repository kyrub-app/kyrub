...
import './ai-development-team-phase0.test';
...
import './admin-integration-readiness.test';
...
test('matching expected fields do not produce a conflict', () => {
  const conflict = buildKyrubConflictEnvelope({ target: { entityType: 'product', entityId: 'product-1' }, expected: { name: 'X-Burger', stock: 8 }, observed: { name: 'X-Burger', stock: 8, privateField: 'not-projected' }, detectedAt: new Date('2026-08-21T12:00:00.000Z') });
  assert.equal(conflict, null);
});

test('state changes produce a bounded STALE_PROPOSAL envelope', () => {
  const conflict = buildKyrubConflictEnvelope({ target: { entityType: 'product', entityId: 'product-1' }, expected: { name: 'X-Burger', stock: 8 }, observed: { name: 'X-Burger Especial', stock: 7, supplierSecret: 'must-never-leak' }, detectedAt: new Date('2026-08-21T12:01:00.000Z') });
  assert.ok(conflict);
...
test('missing entities use the same conflict contract', () => {
  const conflict = buildKyrubConflictEnvelope({ target: { entityType: 'order', entityId: 'order-1' }, expected: { status: 'pending' }, observed: null, detectedAt: new Date('2026-08-21T12:02:00.000Z') });
  assert.ok(conflict);
...
test('assert helper rejects stale execution with a typed 409 conflict', () => {
  assert.throws(() => assertKyrubExpectedState({ target: { entityType: 'order', entityId: 'order-2' }, expected: { status: 'accepted' }, observed: { status: 'completed' }, detectedAt: new Date('2026-08-21T12:03:00.000Z') }), error => {
