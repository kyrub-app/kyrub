...
const validateBeforeQr = async (input: {
  ...
  if (!intentSnapshot.exists || !paymentSnapshot.exists) {
    throw new Error('LOCAL_STORE_PIX_PAYMENT_STATE_MISSING');
  }
  ...
  if (intent.status !== 'pending' || payment.status !== 'pending') {
    throw new Error('LOCAL_STORE_PIX_PAYMENT_NOT_PENDING');
  }
  ...
  if (clean(order.buyerId, 220) !== intent.buyerId) {
    throw new Error('LOCAL_STORE_PIX_BUYER_CHANGED');
  }
  if (
    order.source === 'staff' &&
  ...
  if (paymentContextForOrder(order) !== intent.context) {
    throw new Error('LOCAL_STORE_PIX_CONTEXT_CHANGED');
  }
  ...
  try {
    ...
    throw new Error('LOCAL_STORE_PIX_ORDER_TOTAL_INVALID');
  }
  if (payable.hasOperationalPaidQuantity) {
    throw new Error('LOCAL_STORE_PIX_INTENT_STALE');
  }
  if (orderPayments.size >= MAX_PAYMENT_RECORDS_PER_ORDER) {
    throw new Error('LOCAL_STORE_PIX_PAYMENT_HISTORY_LIMIT');
  }
  ...
  for (const document of orderPayments.docs) {
    ...
    if (candidate.orderId !== intent.target.orderId) {
      throw new Error('LOCAL_STORE_PIX_PAYMENT_SCOPE_INVALID');
    }
    if (candidate.context !== intent.context) {
      throw new Error('LOCAL_STORE_PIX_PAYMENT_CONTEXT_CONFLICT');
    }
    ...
    if (candidate.status === 'pending') {
      throw new Error('LOCAL_STORE_PIX_OTHER_PAYMENT_PENDING');
    }
...
const bindStoreOwnedPix = async (input: {
  canonicalStoreId: string;
  paymentIntentId: string;
  paymentId: string;
  ...
  await adminDb.runTransaction(async transaction => {
    ...
    if (existing && existing !== input.providerPaymentId) {
      throw new Error('LOCAL_STORE_PIX_BINDING_CONFLICT');
    }
    if (!existing) {
...
export const attachStoreOwnedPixToLocalIntent = async (input: {
  ...
  if (!actorUserId || actorUserId !== request.storeId) {
    throw new Error('LOCAL_STORE_PIX_FORBIDDEN');
  }
  ...
  const configuration = await loadStoreOwnedPixConfiguration(request.storeId, {
    requireEnabled: !validated.existingProviderPaymentId,
  });
