...
export const quoteLocalOrderCoupon = async (input: {
  ...
  const snapshot = await adminDb.doc(`stores/${storeContext.canonicalStoreId}/orders/${orderId}`).get();
  if (!snapshot.exists) throw new Error('LOCAL_COUPON_ORDER_NOT_FOUND');
  const order = snapshot.data() as DocumentData;
  if (clean(order.id) !== orderId || order.fulfillmentType !== 'dine_in') throw new Error('LOCAL_COUPON_ORDER_NOT_ELIGIBLE');
  if (order.status === 'rejected' || order.status === 'cancelled') throw new Error('LOCAL_COUPON_ORDER_NOT_ELIGIBLE');
  const buyerId = clean(order.buyerId);
  if (!buyerId || buyerId.startsWith('local-order:')) throw new Error('LOCAL_COUPON_CUSTOMER_IDENTIFICATION_REQUIRED');

  const payable = summarizeLocalOrderPayable(order);
  if (payable.hasOperationalPaidQuantity) throw new Error('LOCAL_COUPON_RECONCILIATION_REQUIRED');
  if (!Array.isArray(order.items) || order.items.length === 0) throw new Error('LOCAL_COUPON_ORDER_ITEMS_INVALID');

  const lines = order.items.map((candidate: unknown) => {
    if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) throw new Error('LOCAL_COUPON_ORDER_ITEMS_INVALID');
    ...
    if (!productId || unitPrice === null || unitPrice < 0 || quantity === null || quantity <= 0) throw new Error('LOCAL_COUPON_ORDER_ITEMS_INVALID');
    return { productId, unitPrice, quantity };
  });

  const resolved = await resolveStorePromotionForCheckout({ storeId: storeContext.canonicalStoreId, buyerId, couponCode, lines });
  if (resolved.quote.discountTotal <= 0) throw new Error('LOCAL_COUPON_NO_DISCOUNT');
  ...
  return {
    ...
    eligibleProductIds: [...resolved.promotion.productIds],
    subtotal,
    discountTotal,
    total,
  };
