...
const cleanId = (value: unknown, max: number): string => {
  ...
  if (!id || id.includes('/') || id.includes('..')) throw new Error('LOCAL_STORE_PIX_CONFIRM_TARGET_INVALID');
  return id;
};

const parseConfirmation = (value: unknown) => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('LOCAL_STORE_PIX_CONFIRM_INVALID');
  const record = value as Record<string, unknown>;
  const allowed = new Set(['storeId', 'paymentIntentId', 'paymentId', 'providerPaymentId', 'confirmedCredit']);
  if (Object.keys(record).some(field => !allowed.has(field))) throw new Error('LOCAL_STORE_PIX_CONFIRM_UNSUPPORTED_FIELD');
  if (record.confirmedCredit !== true) throw new Error('LOCAL_STORE_PIX_CONFIRM_EXPLICIT_ACK_REQUIRED');
  return {
...
const assertPair = (input: {
  ...
  if (
    input.intent.id !== input.paymentIntentId || input.payment.id !== input.paymentId ||
    input.intent.storeId !== input.canonicalStoreId || input.payment.storeId !== input.canonicalStoreId ||
    input.intent.target.kind !== 'existing_order' || input.intent.target.orderId !== input.payment.orderId ||
    input.intent.buyerId !== input.payment.buyerId || input.intent.amount !== input.payment.amount ||
    input.intent.method !== 'pix' || input.payment.method !== 'pix' ||
    ...
    input.intent.provider !== PROVIDER || input.payment.provider !== PROVIDER ||
...
export interface LocalStoreOwnedPixConfirmationResult {
  confirmed: true; duplicate: boolean; paymentIntentId: string; paymentId: string;
  orderId: string; amount: number; currency: 'BRL'; provider: 'store-pix';
  providerPaymentId: string; sourceAuthority: 'operator_attestation';
  bankVerifiedByKyrub: false; attestedAt: string;
}
...
export const confirmStoreOwnedPixLocalPayment = async (input: {
  ...
  if (!actorUserId || actorUserId !== request.storeId) throw new Error('LOCAL_STORE_PIX_CONFIRM_FORBIDDEN');
  ...
  return adminDb.runTransaction(async transaction => {
    const intentRef = adminDb.doc(`stores/${storeContext.canonicalStoreId}/paymentIntents/${request.paymentIntentId}`);
    const paymentRef = adminDb.doc(`stores/${storeContext.canonicalStoreId}/payments/${request.paymentId}`);
    const bindingRef = adminDb.doc(storeOwnedPixPaymentBindingPath(storeContext.canonicalStoreId, request.providerPaymentId));
    const attestationRef = adminDb.doc(attestationPath(storeContext.canonicalStoreId, request.paymentId));
    ...
    const [intentSnapshot, paymentSnapshot, bindingSnapshot, attestationSnapshot, captureSnapshot] = await Promise.all([
      transaction.get(intentRef), transaction.get(paymentRef), transaction.get(bindingRef),
      transaction.get(attestationRef), transaction.get(captureRef),
    ]);
    if (!intentSnapshot.exists || !paymentSnapshot.exists || !bindingSnapshot.exists) {
    ...
    assertPair({
      intent, payment, canonicalStoreId: storeContext.canonicalStoreId,
      paymentIntentId: request.paymentIntentId, paymentId: request.paymentId,
      providerPaymentId: request.providerPaymentId,
    });
    parseStoreOwnedPixPaymentBinding(bindingSnapshot.data(), {
    ...
    if (payment.status === 'paid' && intent.status === 'paid') {
      ...
      if (
        !attestationSnapshot.exists || !captureSnapshot.exists ||
      ...
      return {
        ...
        orderId: payment.orderId, amount: payment.amount, currency: 'BRL', provider: PROVIDER,
        providerPaymentId: request.providerPaymentId, sourceAuthority: 'operator_attestation',
        bankVerifiedByKyrub: false, attestedAt: clean(audit.attestedAt) || payment.paidAt,
      };
    }
    if (payment.status !== 'pending' || intent.status !== 'pending') throw new Error('LOCAL_STORE_PIX_CONFIRM_PAYMENT_NOT_PENDING');
    if (attestationSnapshot.exists || captureSnapshot.exists) throw new Error('LOCAL_STORE_PIX_CONFIRM_RECONCILIATION_REQUIRED');

    // A bound static Pix QR can be paid after the intent's pre-binding creation window.
    // Do not discard a real late credit: the explicit operator attestation is the
    // settlement authority for this mode, analogous to a provider event arriving late.
    const orderRef = adminDb.doc(`stores/${storeContext.canonicalStoreId}/orders/${intent.target.orderId}`);
    const paymentQuery = adminDb.collection(`stores/${storeContext.canonicalStoreId}/payments`)
    ...
    const [orderSnapshot, orderPayments] = await Promise.all([
      transaction.get(orderRef), transaction.get(paymentQuery),
    ]);
    ...
    if (clean(order.buyerId, 220) !== intent.buyerId) throw new Error('LOCAL_STORE_PIX_CONFIRM_BUYER_CHANGED');
    if (contextForOrder(order) !== intent.context) throw new Error('LOCAL_STORE_PIX_CONFIRM_CONTEXT_CHANGED');
    let payable;
    try { payable = summarizeLocalOrderPayable(order); }
    catch { throw new Error('LOCAL_STORE_PIX_CONFIRM_ORDER_TOTAL_INVALID'); }
    if (payable.hasOperationalPaidQuantity || orderPayments.size >= MAX_PAYMENT_RECORDS_PER_ORDER) {
      throw new Error('LOCAL_STORE_PIX_CONFIRM_RECONCILIATION_REQUIRED');
    }
    ...
    for (const document of orderPayments.docs) {
      const compatible = classifyCompatiblePaymentRecord(document.data(), storeContext.canonicalStoreId);
      ...
      if (candidate.status === 'pending') throw new Error('LOCAL_STORE_PIX_CONFIRM_OTHER_PAYMENT_PENDING');
      if (isPaymentAuthoritativelyPaid(candidate.status)) otherPaidAmount += candidate.amount;
    }
    ...
    const audit = {
      ...
      paymentIntentId: intent.id, paymentId: payment.id, orderId: payment.orderId,
      provider: PROVIDER, providerPaymentId: request.providerPaymentId, amount: payment.amount,
      currency: 'BRL', method: 'pix', context: payment.context, actorUserId,
      sourceAuthority: 'operator_attestation', bankVerifiedByKyrub: false, attestedAt,
    ...
    transaction.update(intentRef, { status: 'paid', updatedAt: attestedAt });
    transaction.update(paymentRef, { status: 'paid', paidAt: attestedAt, updatedAt: attestedAt });
    return {
      confirmed: true, duplicate: false, paymentIntentId: intent.id, paymentId: payment.id,
      orderId: payment.orderId, amount: payment.amount, currency: 'BRL', provider: PROVIDER,
      providerPaymentId: request.providerPaymentId, sourceAuthority: 'operator_attestation',
      bankVerifiedByKyrub: false, attestedAt,
    };
  });
};
