...
import { authorizeMercadoLivreBoundListingUpdate } from './mercadoLivreBoundListingUpdateAuthorizationService.js';
...
export const createMercadoLivreRouter = (): Router => {
  ...
  router.post('/:storeId/outbound-update-proposals/:proposalId/authorize', async (request,response)=>{try{const storeId=clean(request.params.storeId);const identity=await authenticatedOwner(request.get('authorization')??'',storeId);response.setHeader('Cache-Control','no-store, max-age=0');response.status(201).json(await authorizeMercadoLivreBoundListingUpdate({storeId,proposalId:clean(request.params.proposalId),authorizedByUserId:identity.uid}));}catch(error){const mapped=mapError(error);response.status(mapped.status).json({error:mapped.message,code:mapped.code});}});
