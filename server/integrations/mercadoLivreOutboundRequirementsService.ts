...
interface AvailableListingTypesResponse {
  category_id?: unknown;
  available?: unknown;
}
...
const predictionsFor = async (storeId: string, siteId: string, title: string) => {
  ...
  return result.map(item => ({
    domainId: clean(item.domain_id, 160),
    domainName: clean(item.domain_name, 160),
...
export const inspectMercadoLivreOutboundRequirements = async (input: {
  ...
  if (!storeId || !proposalId || inspectedByUserId !== storeId) {
    throw new Error('MERCADO_LIVRE_OUTBOUND_REQUIREMENTS_TARGET_INVALID');
  }
...
export const configureMercadoLivreOutboundRequirements = async (input: {
  ...
  if (!suggestions.some(item => item.categoryId === categoryId)) {
  ...
  const [category, categoryAttributes, listingTypes] = await Promise.all([
    ...
    mercadoLivreGetJson<AvailableListingTypesResponse>(
      storeId,
      `/users/${encodeURIComponent(connection.externalAccountId)}/available_listing_types?category_id=${encodeURIComponent(categoryId)}`
    ),
  ]);

  if (clean(category.id, 160) !== categoryId || category.settings?.listing_allowed !== true || clean(category.settings?.status, 80) !== 'enabled') {
  ...
  const selectedListing = available.find(item => {
    if (!item || typeof item !== 'object' || Array.isArray(item)) return false;
    return clean((item as Record<string, unknown>).id, 120) === listingTypeId;
  ...
  const requiredAttributeIds = categoryAttributes
    ...
    .map(attribute => clean(attribute.id, 160))
    .filter(Boolean);
  ...
  await adminDb.runTransaction(async transaction => {
    ...
    if (!currentProposal.exists) throw new Error('MERCADO_LIVRE_OUTBOUND_PROPOSAL_NOT_FOUND');
    const current = assertProposal(storeId, proposalId, currentProposal.data());
    if (current.canonicalBaselineHash !== proposal.canonicalBaselineHash) {
