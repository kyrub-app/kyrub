...
const sameTarget = <T extends HTMLElement>(
  current: T | null,
  next: T | null
): T | null => (current === next ? current : next);
  ...
  useEffect(() => {
    const synchronize = () => {
      ...
      if (editForm) {
        ...
        const profileButton =
          originalNav?.querySelector<HTMLButtonElement>(
        ...
        if (originalNav && profileButton && controls && photoRow) {
          secureShortcuts.forEach(shortcut => {
            ...
            secureButtonsRef.current[shortcut.fullLabel] =
              originalButton;
          ...
          const nextActive =
            activeButton?.getAttribute('aria-label') || 'Perfil';
          ...
          const originalGrid =
            originalNav.firstElementChild as HTMLElement | null;
          if (originalGrid) {
            originalGrid.style.minWidth = '0';
            originalGrid.style.gridTemplateColumns =
              'minmax(104px, 140px)';
          }
          ...
          if (
            contentTarget &&
            contentTarget.previousElementSibling !== photoRow
          ) {
            photoRow.insertAdjacentElement(
              'afterend',
              contentTarget
            );
          }
      ...
      setSecureTarget(current =>
        sameTarget(current, nextSecureTarget)
      );
      ...
      const metricsHeading = [
        ...
        item =>
          item.textContent?.trim() === 'Métricas da publicação'
      );
      const metricsSection =
        metricsHeading?.closest('section') ?? null;
      let nextMetricsSponsorTarget: HTMLElement | null = null;
      let nextSponsoredListTarget: HTMLElement | null = null;

      if (metricsSection) {
        const metricsGrid =
          metricsSection.querySelector<HTMLElement>(
        ...
        createPortal(
          ...
          >
            {secureShortcuts.map(shortcut => {
              ...
              const active =
                activeSecureLabel === shortcut.fullLabel;
      ...
      {sponsoredListTarget &&
        createPortal(
          <button
            ...
            data-empty-label="Nenhuma campanha ativa"
          >
