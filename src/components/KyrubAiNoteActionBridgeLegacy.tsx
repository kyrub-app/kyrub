...
const successMessage = (pending: PendingAction): string => {
  ...
  if (pending.proposal.type === 'create_product') {
    return pending.alreadyApplied
      ? 'Este produto já havia sido cadastrado por esta ação. Nenhuma duplicata foi criada.'
      : 'O produto foi criado pelo executor oficial do Kyrub e será sincronizado no catálogo da sua loja.';
  }
  return pending.alreadyApplied
