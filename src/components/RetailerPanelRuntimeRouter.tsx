...
const MANAGEMENT_MODULES: Record<ErpManagementModule, ModuleDefinition> = {
  ...
  vouchers: {
    title: 'Promocionais',
    description: 'Cupons, pontos, desafios e recompensas em uma única central.',
    status: 'native',
  },
};
...
const mercadoLivreOAuthReturnModule = (): ErpManagementModule | null => {
  ...
  return new URLSearchParams(window.location.search).get('integration') ===
    'mercado_livre'
    ? 'integracoes'
    : null;
};
...
const LazyPromotionalRuntime = lazy(async () => {
  const module = await import('./store/PromotionalDirectRuntime');
  return { default: module.PromotionalDirectRuntime };
});

const LazyCrmRelationshipPanel = lazy(async () => {
  const module = await import('./store/StoreCrmRelationshipPanel');
  return { default: module.StoreCrmRelationshipPanel };
});
  ...
  return (
    ...
    >
      ...
      {moduleId === 'integracoes' ? (
        <Suspense fallback={<Loading>Carregando Integrações & Sandbox…</Loading>}>
          <LazyIntegrationsRuntime triggerToast={retailerProps.triggerToast} />
        </Suspense>
      ) : moduleId === 'produtos' ? (
        <Suspense fallback={<Loading>Carregando Produtos & Estoque…</Loading>}>
        ...
        <Suspense fallback={<Loading>Carregando Promocionais…</Loading>}>
          <LazyPromotionalRuntime />
        </Suspense>
      ) : moduleId === 'crm' ? (
        <Suspense fallback={<Loading>Carregando CRM…</Loading>}>
...
export const RetailerPanel: React.FC<RetailerPanelProps> = props => {
  ...
  useEffect(() => {
    const handleManagementNavigation = (event: Event): void =>
      setManagementModule(
        (event as CustomEvent<ErpManagementNavigationRequest>).detail?.module ??
          null
      );
    ...
    return (
      <section className="rounded-3xl border border-amber-500/25 bg-slate-900 p-5 text-white">
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-slate-400">
          Os módulos de gestão agora são destinos diretos do menu. Esta rota antiga
          permanece apenas como proteção temporária para links legados e não monta o
          painel anterior.
        </p>
        <button
          type="button"
