...
const parseCurrencyMinor = (value: string): number | null => {
  const normalized = value.trim().replace(/\./g, '').replace(',', '.');
  const parsed = Number(normalized);
  const minor = Math.round(parsed * 100);
  return Number.isFinite(parsed) && parsed > 0 && Number.isSafeInteger(minor)
    ? minor
    : null;
};
...
export function StoreChallengeManager() {
    ...
    if (parsedTarget === null) {
      ...
      setMessage(
        metric === 'purchase_count'
          ? 'A meta de compras precisa ser um número inteiro maior que zero.'
          : 'Informe um valor de gasto maior que zero.'
      );
      return;
    }
  ...
  const changeStatus = async (
    ...
    try {
      ...
      setSuccess(true);
      setMessage(`“${challenge.title}” agora está ${statusLabel(status).toLowerCase()}.`);
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
    ...
    <div className="space-y-4" id="store-challenge-manager">
      <section className="rounded-3xl border border-violet-500/20 bg-slate-900 p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-black uppercase text-white">Criar desafio</h4>
              <p className="mt-1 text-[10px] leading-relaxed text-slate-500">
                A loja define a meta e a recompensa. O servidor conta somente pagamentos confirmados e credita Pontos da Loja quando o cliente alcança a meta.
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-1 block font-mono text-[9px] font-black uppercase text-slate-400">Métrica</span>
            <select
              value={metric}
              onChange={event => setMetric(event.target.value as StoreChallengeMetric)}
              disabled={Boolean(busyId)}
              className="w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2.5 text-xs text-white outline-none focus:border-violet-400 disabled:opacity-45"
            >
          ...
          <label className="block">
            <span className="mb-1 block font-mono text-[9px] font-black uppercase text-slate-400">Recompensa em pontos</span>
            <input
              type="number"
              min="1"
              step="1"
          ...
          <label className="block">
            <span className="mb-1 block font-mono text-[9px] font-black uppercase text-slate-400">Duração (dias)</span>
            <input
              type="number"
              min="1"
              max="365"
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900 p-4 sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h4 className="text-xs font-black uppercase text-white">Desafios da loja</h4>
          <span className="text-[9px] font-bold text-slate-500">{challenges.length} registrado{challenges.length === 1 ? '' : 's'}</span>
        </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <strong className="truncate text-xs text-white">{challenge.title}</strong>
                    ...
                    <p className="mt-1 text-[9px] text-slate-500">
                      Meta: {metricLabel(challenge)} · Recompensa: {challenge.rewardPoints} ponto{challenge.rewardPoints === 1 ? '' : 's'} · até {new Date(challenge.endsAt).toLocaleDateString('pt-BR')}
                    </p>
                    {challenge.description && (
                      <p className="mt-1 text-[9px] text-slate-600">{challenge.description}</p>
                    )}
                  </div>
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    ...
                    {challenge.status === 'active' && (
                      ...
                      >
                        {busy ? <LoaderCircle className="h-3 w-3 animate-spin" /> : <CirclePause className="h-3 w-3" />}
                        Pausar
                      </button>
                    )}
