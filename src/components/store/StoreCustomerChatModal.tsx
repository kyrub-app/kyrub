      ...
      <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-slate-900 shadow-2xl sm:h-[min(760px,92dvh)] sm:max-w-lg sm:rounded-3xl sm:border sm:border-slate-800">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-800 bg-slate-950 px-4 pb-4 pt-[max(1rem,env(safe-area-inset-top))] sm:py-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {detail.orderId && (
                <span className="mt-0.5 block truncate font-mono text-[8px] text-orange-400">
                  Contexto: pedido {detail.orderId}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="space-y-3">
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {message.orderId && (
                        <span className={`mt-1 block font-mono text-[7px] ${mine ? 'text-orange-200' : 'text-slate-600'}`}>
              ...
              {!loading && (thread?.messages.length ?? 0) === 0 && (
                <div className="py-12 text-center text-xs leading-relaxed text-slate-500">
                  Esta é a primeira mensagem entre este cliente e esta loja.
                  O histórico continuará no mesmo thread nas próximas compras.
                </div>
              )}
              <div ref={endRef} />
            </div>
        ...
        >
          <input
            ...
            placeholder={detail.perspective === 'store' ? 'Responder como loja…' : 'Escreva para a loja…'}
            className="min-w-0 flex-1 rounded-xl border border-slate-800 bg-slate-900 px-3.5 py-3 text-xs text-white outline-none placeholder:text-slate-600 focus:border-orange-500"
          />
          <button
            type="submit"
