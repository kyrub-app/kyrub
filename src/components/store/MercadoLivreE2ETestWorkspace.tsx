...
import {
  ...
  confirmMercadoLivreE2EVariantIdentity,
...
export default function MercadoLivreE2ETestWorkspace({ user, storeId, connectionId, notify }: {
  user: User;
  storeId: string;
  connectionId: string;
  ...
  const [variantIdentityConfirmed, setVariantIdentityConfirmed] = useState(false);
  const [variantDimensionCount, setVariantDimensionCount] = useState(0);
  ...
  const selectedVariationCount = useMemo(() => {
    if (!options || options.publicationModel !== 'user_products') return 0;
    return options.attributes.filter(attribute => {
      if (!attribute.variationAttribute && !attribute.allowVariations) return false;
      const value = attributeValues[attribute.id];
      return Boolean(value?.valueId?.trim() || value?.valueName?.trim());
    }).length;
  ...
  const invalidateValidation = (): void => {
    ...
    setVariantIdentityConfirmed(false);
    setVariantDimensionCount(0);
    setPublicationAuthorization(null);
  };

  const resetFlow = (product?: MercadoLivreE2EEligibleProduct | null): void => {
    setProposalId(''); setSuggestions([]); setCategoryId(''); setOptions(null); setListingTypeId(''); setCondition(''); setFamilyName(product?.name ?? ''); setAttributeValues({});
    setPublicationReady(false); setCatalogEnriched(false); setCatalogEnrichmentCount(0); setVariantIdentityConfirmed(false); setVariantDimensionCount(0); setPublicationAuthorization(null); setPublicationExecution(null); setPublicationReconciled(false);
  ...
  const loadProducts = async (): Promise<void> => {
    ...
    } catch (error) { setMessage(errorText(error, 'Não foi possível listar os produtos elegíveis.')); }
  ...
  const preparePublication = async (): Promise<void> => {
    ...
    try {
      ...
      setProposalId(proposal.id); setSuggestions(inspection.categorySuggestions);
      if (!familyName.trim()) setFamilyName(selectedProduct.name);
      setMessage('Proposta criada sem publicar nada. Escolha uma das categorias sugeridas oficialmente pelo Mercado Livre.');
    } catch (error) { setMessage(errorText(error, 'Não foi possível preparar a publicação.')); }
  ...
  const chooseCategory = async (nextCategoryId: string): Promise<void> => {
    ...
    try {
      ...
      setOptions(result);
      if (result.publicationModel === 'user_products' && !familyName.trim()) setFamilyName(selectedProduct?.name ?? '');
      setMessage(result.publicationModel === 'user_products'
        ? 'Esta conta usa User Products. Defina a família, revise as variações e complete as características oficiais antes de validar.'
        : 'Opções oficiais carregadas. O Cairube organizou características, disponibilidade, garantia e entrega em etapas; informações opcionais podem ser puladas.');
    } catch (error) { setMessage(errorText(error, 'Não foi possível carregar as opções da categoria.')); }
    finally { setBusy(false); }
  };
  ...
  const validatePublication = async (commercial: MercadoLivreCommercialWizardInput): Promise<void> => {
    ...
    if (options?.publicationModel === 'user_products' && !familyName.trim()) {
      setMessage('Informe o nome da família antes de validar este User Product.');
      return;
    }
    ...
    try {
      const configured = await configureMercadoLivreE2ERequirements(user, storeId, proposalId, { categoryId, listingTypeId, condition, familyName: familyName.trim() || undefined, attributes });
      ...
      if (validation.status !== 'ready_for_owner_authorization') {
        ...
        const localHint = commercialConfigured.missingRequiredSaleTermIds.length
          ...
          : '';
        setMessage(causes ? `O Mercado Livre pediu correções: ${causes}.${localHint} Revise as etapas do wizard e valide novamente.` : `O anúncio ainda precisa de correções antes de publicar.${localHint} Revise as etapas do wizard.`);
        return;
      }
      setPublicationReady(true);
      setMessage(options?.publicationModel === 'user_products' && selectedVariationCount > 0
        ? 'Validação concluída sem criar anúncio. A família e as dimensões de variante podem ser confirmadas no catálogo Kyrub; publicar continua exigindo autorização separada.'
        : 'Validação concluída sem criar anúncio. Você pode opcionalmente salvar as características confirmadas no produto Kyrub; publicar continua exigindo autorização separada.');
  ...
  const confirmVariantIdentity = async (): Promise<void> => {
    if (!proposalId || !publicationReady || options?.publicationModel !== 'user_products' || selectedVariationCount === 0) return;
    setBusy(true);
    try {
  ...
  const authorizePublication = async (): Promise<void> => {
    ...
    } catch (error) { setMessage(errorText(error, 'Não foi possível autorizar a publicação.')); }
    finally { setBusy(false); }
  };
  ...
  const publishNow = async (): Promise<void> => {
    ...
    setBusy(true);
    try {
      const result = await executeMercadoLivreE2EPublication(user, storeId, publicationAuthorization.id, publicationAuthorization.token);
      setPublicationAuthorization(null);
      setPublicationExecution({ id: result.executionId, bindingId: result.bindingId, externalItemId: result.externalItemId, permalink: result.permalink });
      setBindingId(result.bindingId); setExternalItemId(result.externalItemId);
    ...
    } catch (error) { setPublicationAuthorization(null); setMessage(errorText(error, 'A execução ficou sem confirmação segura. Use o fluxo de reconciliação.')); }
    finally { setBusy(false); }
  };
  ...
  const reconcilePublication = async (): Promise<void> => {
    ...
    try {
      ...
      setBindingId(result.bindingId); setExternalItemId(result.externalItemId); setPublicationReconciled(true);
      setMessage('Publicação reconciliada com o GET autoritativo do Mercado Livre. O binding está pronto para o teste de estoque.');
    } catch (error) { setMessage(errorText(error, 'Ainda não foi possível reconciliar a publicação.')); }
    finally { setBusy(false); }
  };

  const savePolicy = async (): Promise<void> => {
    if (!selectedProduct || !policyEnabledChoice || safetyStock.trim() === '') { setMessage('Defina explicitamente se o canal ficará habilitado e qual será o estoque de segurança.'); return; }
    const safety = Number(safetyStock); const cap = allocationCap.trim() === '' ? null : Number(allocationCap);
    if (!Number.isSafeInteger(safety) || safety < 0 || (cap !== null && (!Number.isSafeInteger(cap) || cap < 0))) { setMessage('Estoque de segurança e teto precisam ser números inteiros não negativos.'); return; }
    setBusy(true);
    try {
      await setMercadoLivreE2EAvailabilityPolicy(user, selectedProduct.canonicalStoreId, { enabled: policyEnabledChoice === 'enabled', safetyStockUnits: safety, allocationCapUnits: cap });
      setPolicySaved(true); setAvailabilitySnapshot(null); setStockProposal(null);
      setMessage('Política salva. Ela define a projeção do canal; não altera o estoque físico.');
    } catch (error) { setMessage(errorText(error, 'Não foi possível salvar a política de disponibilidade.')); }
    finally { setBusy(false); }
  };
  ...
  const createSnapshot = async (): Promise<void> => {
    ...
    try {
      ...
      setAvailabilitySnapshot({ id: result.snapshotId, publishableUnits: result.publishableUnits, availableToPromiseUnits: result.availableToPromiseUnits });
      setStockProposal(null); setMessage(`Snapshot congelado: ATP ${result.availableToPromiseUnits}; publicável no Mercado Livre ${result.publishableUnits}.`);
    } catch (error) { setMessage(errorText(error, 'Não foi possível calcular a disponibilidade do canal.')); }
    finally { setBusy(false); }
  };
  ...
  const proposeStock = async (): Promise<void> => {
    ...
    try {
      ...
      setStockProposal({ id: result.id, status: result.status, target: result.targetAvailableQuantity, observed: result.observedAvailableQuantity, blockedReason: result.blockedReason });
      setMessage(result.status === 'no_changes' ? 'O Mercado Livre já expõe a quantidade projetada. Nenhum PUT é necessário.' : result.status === 'blocked_provider_stock_mode' ? `Atualização bloqueada com segurança: ${result.blockedReason}.` : 'Proposta de estoque criada. Ainda não houve PUT no Mercado Livre.');
    } catch (error) { setMessage(errorText(error, 'Não foi possível criar a proposta de estoque.')); }
    finally { setBusy(false); }
  };
  ...
  const authorizeStock = async (): Promise<void> => {
    ...
    setBusy(true);
    try { const result = await authorizeMercadoLivreE2EStock(user, storeId, stockProposal.id); setStockAuthorization({ id: result.authorizationId, token: result.authorizationToken }); setMessage('Autorização one-time de estoque criada. Ela ainda não alterou o Mercado Livre.'); }
    catch (error) { setMessage(errorText(error, 'Não foi possível autorizar a atualização de estoque.')); }
    finally { setBusy(false); }
  };
  ...
  const updateStockNow = async (): Promise<void> => {
    ...
    setBusy(true);
    try { const result = await executeMercadoLivreE2EStock(user, storeId, stockAuthorization.id, stockAuthorization.token); setStockAuthorization(null); setStockExecutionId(result.executionId); setMessage(`PUT executado uma vez. Alvo enviado: ${result.targetAvailableQuantity}. Agora confirme por GET autoritativo.`); }
    catch (error) { setStockAuthorization(null); setMessage(errorText(error, 'A escrita de estoque ficou sem confirmação segura. Reconcilie antes de qualquer nova tentativa.')); }
    finally { setBusy(false); }
  };
  ...
  const reconcileStock = async (): Promise<void> => {
    ...
    setBusy(true);
    try { const result = await reconcileMercadoLivreE2EStock(user, storeId, stockExecutionId); setStockReconciled(true); setMessage(`Estoque reconciliado: Mercado Livre confirmou ${result.observedAvailableQuantity} unidade(s). E2E concluído.`); notify('E2E Mercado Livre concluído com publicação e estoque reconciliados.', 'success'); }
    catch (error) { setMessage(errorText(error, 'O estoque ainda não foi confirmado pelo Mercado Livre.')); }
    finally { setBusy(false); }
  };

  const stockStageAvailable = Boolean(selectedProduct && bindingId && (selectedProduct.activeBindingId || publicationReconciled));

  return <section className="rounded-3xl border border-cyan-500/20 bg-cyan-500/5 p-5">
