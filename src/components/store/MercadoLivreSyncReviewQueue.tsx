...
export default function MercadoLivreSyncReviewQueue({
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    setError('');
    try {
      const result = await loadMercadoLivreSyncReviewQueue(user, storeId, 50);
  ...
  const decide = async (
    ...
    try {
      ...
      notify(
        decision === 'approve'
          ? 'Proposta aprovada para uma etapa posterior de aplicação. O catálogo Kyrub ainda não foi alterado.'
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-slate-500">
            Estes dados foram reconsultados pela API oficial depois da notificação. Aprovar aqui registra sua decisão, mas ainda não altera produto, preço, estoque ou publicação no Kyrub.
          </p>
        </div>
        <button
          type="button"
