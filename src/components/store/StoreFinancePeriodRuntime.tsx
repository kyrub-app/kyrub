...
const payableCategoryLabel = (category: StorePayableCategory): string => ({
  supplier: 'Fornecedor',
...
async function fetchPeriod(storeId: string, period: string): Promise<PeriodPayload> {
  ...
  if (!contentType.includes('application/json')) {
    throw new Error('A visão por período respondeu em um formato inesperado.');
  }
    ...
    <section className="min-w-0 max-w-full overflow-hidden rounded-3xl border border-indigo-500/20 bg-slate-900 p-5 text-white" data-kyrub-finance-period-view="complete-paged-scan">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-[9px] leading-relaxed text-slate-400">
            A competência é apurada no servidor percorrendo todos os lotes do período. Ela não depende mais dos primeiros 100 lançamentos do Financeiro. Nenhum valor ausente é estimado.
          </p>
        </div>
      ...
      {recoveredCount > 0 && (
        <p className="mt-3 rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-3 text-[9px] text-cyan-100">
          {recoveredCount} pagamento(s) histórico(s) pago(s) foram reconciliados antes da apuração, usando IDs determinísticos para não duplicar receita.
        </p>
      )}

      {loading ? (
        <p className="mt-4 rounded-2xl border border-dashed border-slate-700 p-4 text-center text-[9px] text-slate-500">Apurando todos os lotes da competência…</p>
      ) : error ? (
        ...
        <>
          ...
          <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-2">
            <article className="min-w-0 rounded-2xl border border-slate-800 bg-slate-950 p-4">
              ...
              <p className="mt-3 text-[8px] leading-relaxed text-slate-600">
                Não inclui CMV/consumo de estoque, depreciação, tributos não lançados, taxas não evidenciadas ou outras competências ainda não registradas. Portanto, não é apresentado como lucro líquido contábil ou DRE oficial.
              </p>
            </article>
            ...
            <article className="min-w-0 rounded-2xl border border-slate-800 bg-slate-950 p-4">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <div className="rounded-xl border border-slate-800 bg-slate-900 p-3"><span className="text-[8px] font-black uppercase text-slate-600">Contas em aberto com vencimento no mês</span><strong className="mt-1 block text-sm text-amber-200">{money(report.openDueMinor)}</strong><span className="text-[7px] text-slate-600">{report.openDueCount} conta(s)</span></div>
              ...
              {report.categories.length > 0 && (
                <div className="mt-3 space-y-1.5 border-t border-slate-800 pt-3">
                  ...
                  {report.categories.map(item => (
                    <div key={item.category} className="flex justify-between gap-3 text-[8px]"><span className="text-slate-500">{payableCategoryLabel(item.category)}</span><b>{money(item.amountMinor)}</b></div>
                  ))}
                </div>
              )}
            </article>
          ...
          <p className="mt-4 rounded-2xl border border-slate-800 bg-slate-950 p-3 text-[8px] leading-relaxed text-slate-600">
            O Caixa físico/operacional continua exibido separadamente no Financeiro. Ele não é incorporado a este resultado para evitar dupla contabilização com pagamentos e Contas a Pagar.
          </p>
        </>
      ) : null}
    </section>
