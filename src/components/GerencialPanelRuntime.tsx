...
import { persistPublicProduct, type PublicProduct } from '../utils/publicProducts';
...
function ModuleCard({
  ...
  return (
    ...
    >
      ...
      <span
        className={`flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-800 bg-slate-950 ${accent}`}
      >
    ...
    <section className="space-y-5 rounded-3xl border border-slate-800 bg-slate-900 p-5 sm:p-6">
      <div>
        ...
        <h3 className="mt-1 text-base font-black text-white">
          Desempenho registrado pela loja
        </h3>
...
function IntegrationsPanel({
  ...
  const [notice, setNotice] = useState<{ message: string; type: ToastType } | null>(
    null
  );
  ...
  return (
    <div className="space-y-5" id="kyrub-gerencial-integrations-runtime">
      {notice && (
        <div
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900 p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
          {['99Food', 'iFood', 'Shopee', 'Amazon Brasil', 'Quita', 'Kyrub Marketplace Hub'].map(
            channel => (
              <div
                key={channel}
...
export function GerencialPanel({
  ...
  return (
    <div
      ...
      data-kyrub-gerencial-runtime="inert-entry"
    >
      <header className="flex flex-col gap-3 rounded-3xl border border-slate-800 bg-slate-900 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2 py-0.5 font-mono text-[8px] font-black uppercase text-emerald-300">
              Runtime nativo
            </span>
          </div>
          ...
          <p className="mt-1 text-[10px] leading-relaxed text-slate-500">
            A entrada desta área não inicia listeners, modais ou bridges. Cada módulo ativa somente o que precisa.
          </p>
        </div>
      ...
      {!activeModule && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <ModuleCard
            ...
            description="Equipe, cargos, acessos e rotinas de trabalho da loja única do usuário."
            icon={Users}
      ...
      {activeModule === 'integracoes' && (
        <IntegrationsPanel triggerToast={triggerToast} />
      )}
