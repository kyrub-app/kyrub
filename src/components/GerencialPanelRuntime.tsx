...
const LazyPromotionsManager = lazy(async () => {
  const module = await import('./StorePromotionsManager');
...
function MigrationNotice({ title }: { title: string }) {
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900 p-5 text-white">
      <span className="font-mono text-[9px] font-black uppercase tracking-[0.16em] text-orange-300">Runtime isolado</span>
      <h3 className="mt-2 text-sm font-black">{title}</h3>
      <p className="mt-2 text-[10px] leading-relaxed text-slate-400">Este módulo ainda não foi reativado neste runtime. Ele será ligado em um corte próprio, sem reintroduzir o painel legado que congelava a aplicação.</p>
    </section>
  );
}
...
export function GerencialPanel({ activeRetailerId, products, triggerToast, setActiveSubTab }: GerencialPanelProps) {
  ...
  return (
    <div id="kyrub-gerencial-native-runtime" data-kyrub-gerencial-runtime="lazy-shell" className="space-y-5">
      <header className="rounded-3xl border border-emerald-500/25 bg-slate-900 p-5 text-white">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-lg font-black">{activeModule ? TITLES[activeModule] : 'Gestão da loja'}</h2>
            <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-slate-400">A raiz do Gerencial não carrega Firebase, catálogo, pedidos, Mercado Livre ou bridges. Cada módulo nasce somente depois do clique.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {activeModule && <button type="button" onClick={() => setActiveModule(null)} className="min-h-10 rounded-xl border border-slate-700 bg-slate-950 px-3 text-[9px] font-black uppercase text-slate-300">Menu Gerencial</button>}
            <button type="button" onClick={() => setActiveSubTab('clientes')} className="min-h-10 rounded-xl bg-orange-500 px-3 text-[9px] font-black uppercase text-slate-950">Voltar ao PDV</button>
          </div>
        </div>
      </header>
      ...
      {!activeModule && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2" id="kyrub-gerencial-module-grid">
          {MODULES.map(module => (
            <button key={module.id} type="button" disabled={module.disabled} onClick={() => {
              if (!module.disabled && module.id !== 'crm' && module.id !== 'marketing') setActiveModule(module.id);
            }} className={`relative min-h-32 rounded-3xl border border-slate-800 bg-slate-900 p-5 text-left text-white ${module.disabled ? 'cursor-not-allowed opacity-65' : 'cursor-pointer hover:border-orange-500/35'}`}>
              {module.badge && <span className="absolute right-4 top-4 rounded-full border border-slate-700 bg-slate-950 px-2 py-1 font-mono text-[8px] font-black uppercase tracking-wider text-slate-400">{module.badge}</span>}
              <h3 className="pr-24 text-xs font-black uppercase">{module.title}</h3>
              <p className="mt-2 text-[10px] leading-relaxed text-slate-400">{module.description}</p>
            </button>
          ))}
        </div>
      )}
      ...
      {activeModule === 'integracoes' && (
        <Suspense fallback={<div className="rounded-3xl border border-cyan-500/20 bg-slate-900 p-5 text-[10px] text-cyan-100">Carregando Integrações & Sandbox…</div>}>
          <LazyIntegrationsRuntime triggerToast={triggerToast} />
        </Suspense>
      )}

      {activeModule === 'vouchers' && (
        <Suspense fallback={<div className="rounded-3xl border border-orange-500/20 bg-slate-900 p-5 text-[10px] text-orange-100">Carregando Cupons & Vouchers…</div>}>
          <LazyPromotionsManager storeId={activeRetailerId} products={products} triggerToast={triggerToast} />
        </Suspense>
      )}
